"""
Waters Acquity UPC Driver

This module provides a Python interface for controlling Waters Acquity UPC equipment
using the Automation Portal PC Protocol Specification.
"""
...
class WatersAcquityError(Exception):
    """Custom exception for Waters Acquity communication errors."""
    pass


class WatersAcquityDriver:
    """
    Driver for Waters Acquity UPC equipment using Automation Portal Protocol.
    
    This class provides methods to communicate with and control Waters Acquity
    Ultra Performance Convergence Chromatography (UPC) systems via serial or TCP/IP.
    ...
    def send_command(self, command: str, retries: int = None) -> str:
        ...
        Raises:
            WatersAcquityError: If communication fails
        """
        if not self.is_connected or not self.connection:
            raise WatersAcquityError("Not connected to Waters Acquity system")
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if self.comm_mode == config.COMM_MODE_SERIAL:
                    ...
                    raise WatersAcquityError(f"Invalid communication mode: {self.comm_mode}")
                
                self.logger.debug(f"Sent: {command} | Received: {response}")
                
                # Check for errors in response
                if self._is_error_response(response):
                    raise WatersAcquityError(f"Instrument error: {response}")
                ...
                if attempt < retries:
                    ...
                    raise WatersAcquityError(f"Communication error after {retries + 1} attempts: {e}")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.is_connected:
            try:
                ...
                except (ValueError, WatersAcquityError):
                    pass
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    ...
                    raise WatersAcquityError(f"Portal command failed after {retries + 1} attempts: {e}")
...
def example_context_manager():
    ...
    with WatersAcquityDriver(port='COM1') as driver:
        status = driver.get_status()
        print(f"System status: {status}")
